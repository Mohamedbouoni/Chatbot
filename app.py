...
# Import the appropriate verifier based on environment
USE_HUGGINGFACE = os.getenv("USE_HUGGINGFACE", "false").lower() == "true"

if USE_HUGGINGFACE:
    print("🌐 Using Hugging Face API for cloud deployment")
    from app.services import verifier_cloud as verifier
else:
    print("🏠 Using local Ollama for verification")
    from app.services import verifier

app = FastAPI(
    title="TruthBot AI - Fact Verification",
    description="Fact-checking using AI and Serper Dev",
    version="3.0.0"
)
...
@app.get("/health")
def health_check():
    ai_engine = "Hugging Face (Mistral-7B)" if USE_HUGGINGFACE else os.getenv("OLLAMA_MODEL", "qwen")
    return {
        "status": "healthy",
        "service": "TruthBot AI",
        "version": "3.0.0",
        ...
        "deployment": "cloud" if USE_HUGGINGFACE else "local"
    }
